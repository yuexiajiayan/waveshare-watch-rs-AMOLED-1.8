...
async fn ntp_sync(
    ...
    let ntp_addr = embassy_net::Ipv4Address::new(116,62,13,223);
    ...
    match embassy_time::with_timeout(
        ...
        Ok(Ok((len, _addr))) if len >= 48 => {
            ...
            // Convert to hours/minutes/seconds (Beijing time, UTC+8)
            let utc_offset = 8 * 3600;
...
async fn main(_spawner: Spawner) {
    ...
    let mut expander = Tca9554::new(RefCellDevice::new(&i2c_ref));
    let _ = expander.init();
    let _ = expander.set_output(board::LCD_RESET, false);
    let _ = expander.set_output(board::DSI_PWR_EN, false);
    let _ = expander.set_output(board::TP_RESET, false);
    delay.delay_millis(20);
    let _ = expander.set_output(board::LCD_RESET, true);
    let _ = expander.set_output(board::DSI_PWR_EN, true);
    let _ = expander.set_output(board::TP_RESET, true);
    println!("[EXIO] OK");
    ...
    let reset = expander.into_pin(board::LCD_RESET, true).expect("EXIO LCD reset failed");
    ...
    // === Touch ===
    let mut touch_rst = expander.into_pin(board::TP_RESET, true).expect("EXIO touch reset failed");
    // GPIO21 is the FT3168 INT line: held high by pull-up, pulled low by the controller
    // when a finger is on the screen. We use it both for level checks and as an async wake source.
    let mut touch_int = Input::new(peripherals.GPIO21, InputConfig::default().with_pull(Pull::Up));
    ...
    let sd_cs = expander.into_pin(board::SD_CS, true).expect("EXIO SD CS failed");
    ...
    // Pre-generate beep sound (800Hz, 180ms, stereo 16-bit @ 8kHz = 5760 bytes incl. slack)
    static mut BEEP_BUF: [u8; 6000] = [0u8; 6000];
    static mut WORKS_PLAY_BUF: [u8; 8192] = [0u8; 8192];
    let beep_len = fill_beep_buffer(unsafe { &mut BEEP_BUF }, 800, 8000, 180);
    println!("[AUDIO] I2S configured for 8kHz / 16-bit / channel16 format");
    ...
    let wifi_ssid = "1hao";//option_env!("WIFI_SSID").unwrap_or("");
    let wifi_pass = "yibajiuling";//option_env!("WIFI_PASS").unwrap_or("");
    ...
    watchface.wifi_connected = false; // radio starts disconnected, then auto-connects at boot if credentials exist
    ...
    //   * the touch interrupt line (GPIO21, async falling edge)
    ...
    let mut wifi_on_request: bool = true;       // default on at boot
    ...
    // Latches a touch IRQ edge so we read the controller even if INT is only a pulse.
    let mut touch_irq_pending = false;
    ...
    loop {
        ...
        let touch_tracking = touch.is_tracking() || touch_irq_pending;
        let button_held = boot_button.is_low();

        let tick = if touch_held || touch_tracking || button_held {
            ...
            match app_state {
                ...
                | AppState::SmartHome => Duration::from_millis(33),
        ...
        let timer_fut = Timer::after(tick);
        let touch_fut = touch_int.wait_for_falling_edge();
        let button_fut = boot_button.wait_for_falling_edge();
        let wake = select3(timer_fut, touch_fut, button_fut).await;
        let touch_edge = matches!(wake, embassy_futures::select::Either3::Second(_));
        if touch_edge {
        ...
        //   1. a touch IRQ edge fired, OR
        //   2. a finger is currently on the panel, OR
        //   3. the finger was on the panel last iteration (catches the
        ...
        let touch_active = screen_state >= 2 && (touch_irq_pending || int_low || was_touching);
        ...
        match app_state {
                    ...
                    AppResult::Continue => {
                        if snake_game.stepped() {
                            ...
                            if snake_game.score() > prev_score {
                                println!("[AUDIO] Snake food beep trigger: score {} -> {}", prev_score, snake_game.score());
                                ...
                                if let Ok(transfer) = i2s_tx.write_dma(unsafe { &BEEP_BUF }) {
                                    println!("[AUDIO] Snake beep DMA start ({} bytes valid)", beep_len);
                                    let _ = transfer.wait();
                                    println!("[AUDIO] Snake beep DMA done");
                                } else {
                                    println!("[AUDIO] Snake beep DMA start failed");
                                }
            ...
            AppState::Mp3Player => {
                let was_playing = mp3_player.state() == PlayerState::Playing;
                let input = AppInput { touch: None, swipe: swipe_event, tap: tap_event, accel, dt_ms: dt_ms.max(1) };
                mp3_player.update(&input);
                if !was_playing && mp3_player.state() == PlayerState::Playing {
                    println!("[MP3] Playing IT_WORKS ({} bytes)", works::IT_WORKS.len());
                    let _ = audio_codec.unmute();
                    let _ = audio_codec.set_volume(0xA5);
                    delay.delay_millis(2);
                    pa_en.set_high();
                    let play_buf = unsafe { &mut WORKS_PLAY_BUF };
                    play_buf.fill(0);
                    match i2s_tx.write_dma_circular(play_buf) {
                        Ok(mut transfer) => {
                            let mut pos = 0usize;
                            while pos + 1 < works::IT_WORKS.len() {
                                match transfer.available() {
                                    Ok(avail) if avail >= 4 => {
                                        let writable = avail.min((works::IT_WORKS.len() - pos) * 2);
                                        let writable = writable - (writable % 4);
                                        if writable == 0 {
                                            continue;
                                        }
                                        let _ = transfer.push_with(|buf| {
                                            let limit = buf.len().min(writable) - (buf.len().min(writable) % 4);
                                            let mut out = 0usize;
                                            while out + 3 < limit && pos + 1 < works::IT_WORKS.len() {
                                                let lo = works::IT_WORKS[pos];
                                                let hi = works::IT_WORKS[pos + 1];
                                                buf[out] = lo;
                                                buf[out + 1] = hi;
                                                buf[out + 2] = lo;
                                                buf[out + 3] = hi;
                                    ...
                                    Ok(_) => {}
                                    Err(_) => {
                                        println!("[MP3] IT_WORKS DMA stream failed at {}", pos);
                                        break;
                                    }
                                }
                            }
                        ...
                        Err(_) => {
                            println!("[MP3] IT_WORKS circular DMA start failed");
                        }
                    }
                    pa_en.set_low();
                    let _ = audio_codec.mute();
                    mp3_player.set_state(PlayerState::Stopped);
                    println!("[MP3] IT_WORKS done");
                }
